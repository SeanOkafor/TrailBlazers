...
public class TutorialEnemy {
	...
	// Collision insets — the actual block content is centred inside the
	// 395x395 display area with transparent padding around it.
	// These shrink the hitbox to match the visible sprite (~195x195).
	private static final int COLLISION_INSET_X = 100;  // left & right padding
	private static final int COLLISION_INSET_Y = 100;  // top & bottom padding
	...
	public void draw(Graphics2D g2d) {
		...
		if (frames[frameIndex] != null) {
			...
			// --- Damage flash: tint only the visible sprite pixels red ---
			// Instead of drawing a full rectangle (which covers transparent areas),
			// we create a temporary image, draw the sprite into it, then composite
			// a red fill using SRC_IN so only the opaque sprite pixels turn red.
			if (damageFlashTimer > 0) {
				...
				// Draw the sprite into the temp buffer (establishes alpha mask)
				flashG.drawImage(frames[frameIndex], 0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT, null);
				// Composite red ONLY where the sprite has opaque pixels
				...
				// Draw the red-tinted sprite on top with partial transparency
	...
	public boolean collidesWith(Projectile p) {
		...
		// AABB overlap check (using insets so the hitbox matches the visible sprite,
		// not the full 395x395 display area which includes transparent padding)
